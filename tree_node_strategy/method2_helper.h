...
static AB_node *AddKeyValue_to_Bottom(AB_Tree *self, AB_node *cur, KV_pair kv, int new_key_pos);
static AB_node *AddKeyLink_to_internal_node(AB_Tree *self, node_arg dst_node, node_arg src_node);
...
static inline int GetKeyRank(AB_Tree *self, AB_node *node, const void *key)
{
    ...
    while(l <= r)
    {
        ...
        int cmp = AB_CompareKey(self, key, node, (l+r)>>1);
...
static AB_node *AddKeyValue_to_Bottom(AB_Tree *self, AB_node *cur, KV_pair kv, int new_key_pos)
{
    if (IsFullNode(self, cur))
    {
        ...
        if (new_key_pos >= (self->b)/2)//new item will reside in the new node
        {
...
static AB_node *AddKeyLink_to_internal_node(AB_Tree *self, node_arg dst_node, node_arg src_node)
{
    ...
    if (IsFullNode(self, dst_node.node))
    {
        ...
        if (dst_node.ith_element >= (self->b)/2)//new item will reside in the new node
        {
