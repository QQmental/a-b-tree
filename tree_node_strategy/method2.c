...
*/
static void ReplacedWithPredcesor(AB_Tree *self, AB_node *node_dst, int KeyRank_dst);
...
void AB_tree_Insert_algo(AB_Tree *self, KV_pair kv)
{
    ...
    new_node = AddKeyValue_to_Bottom(self, cur, kv, self->SearchBranch[level]);
    ...
    while(level != -1 && new_node)
    {
        ...
        new_node = AddKeyLink_to_internal_node(self, Setnode_arg(self->SearchPath[level], self->SearchBranch[level]), Setnode_arg(self->SearchPath[level+1], self->SearchPath[level+1]->KeyCount - 1));
...
int AB_tree_Delete_algo(AB_Tree *self, const void *key)
{
    ...
    {
        ReplacedWithPredcesor(self, node_key, rank_key);
    }
...
void GetTotalKeytCount(AB_Tree *self)
{
    self->SearchPathLength = 1 ;
    self->SearchPath[0] = self->root;
    int i = 0, level = 0;
    ...
    while(level >= 0)
    {
        if (self->SearchPath[level]->IsBottom || self->SearchBranch[level] > self->SearchPath[level]->KeyCount)
        {
            cnt += self->SearchPath[level]->KeyCount;
    ...
    printf("total unique keyCount = %d\n",cnt);
}

void GetTotalNodeCount(AB_Tree *self)
{
    self->SearchPathLength = 1 ;
    self->SearchPath[0] = self->root;
    int i = 0, level = 0;
    ...
    printf("total unique NodeCount = %d\n",cnt);
}